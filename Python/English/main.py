...
def certificate(cor: int, inc: int, all_q: int) -> None:
    ...
    print(f'+{"-" * 39}+')
    print(
        f"| There were only a few questions: {colored(all_q, 'yellow')}  {' ' * (3 - len(str(all_q)))}|")
    print(
        f"|          Correct answers: {colored(cor, 'green')}         {' ' * (3 - len(str(cor)))}|")
    print(
        f"|          Incorrect answers: {colored(inc, 'red')}       {' ' * (3 - len(str(inc)))}|")
    perc = round((cor / all_q) * 100, 1)
    print(f"| Percentage of correct answers: {colored(f'{perc}', 'light_blue')}%{
          abs(len(str(perc)) - 6) * ' '}|")
    exec_time = f"{((time.time() - start) / 60):.3f}"
    print(
        f'|       Execution time: {colored(exec_time, "yellow")} min    {" " * (4 - len(exec_time[: -4]))}|')
    print(f'+{"-" * 39}+')
...
def main() -> None:
    ...
    answer = input(
        colored("Enter three irregular verb forms: to ", "light_blue"))
...
try:
    if __name__ == "__main__":
        ...
        print(
            colored("If you can use two words in the same form, use: '/'", "light_magenta"))
