...
def man(ind): # Возвращает состояние персонажа
    falls_list = [
                # начальное состояние
                '''
                   --------
    ...
    return falls_list[ind]


def rand_word(): # Генерирует рандомное слово из списка
...
def foolproof(str_): # Защита от дурака (на входную букву)
    alphas = [chr(i) for i in range(ord('а'), ord('я') + 1)]
    while len(str_) != 1 or str_ not in alphas or str_ in used_letters:
        print(colored('Введено неверное значени', 'light_red'))
        print(colored('Использованные буквы:', 'yellow') + used_letters)
        str_ = input(colored('Введите строчную букву (1 штуку), которая еще не была использованна:\n', 'light_red'))
    return str_


def game(): # Главная логика игры
    global used_letters

    hidden_word = []
    rand_w = rand_word()
    hidden_word.extend(rand_w)
    rand_w = hidden_word[:]

    now_word = ['_' for i in range(len(hidden_word))]
    falls = 0

    while True:

        print(colored('Здоровье', 'red'), f'{'❤️' * (7 - falls)} ({colored(str(7 - falls), 'red')})')
        print(colored('Слово на данный момент: ', 'light_cyan'), *now_word, sep='')
        print(colored(f'Использованные буквы: {used_letters}', 'yellow'))
        question = input(colored('Попробуйте угадать одну из букв слова:\n', 'yellow'))
        question = foolproof(question)

        if question in hidden_word:

            while question in hidden_word:
                index = hidden_word.index(question)
                now_word[index] = question
                hidden_word[index] = '_'
                used_letters += question

        elif not question in hidden_word:
            print(colored('Состояние вашего персонажа:', 'light_cyan'))
            print(colored(man(falls), 'light_red'))
            falls += 1
            used_letters += question

        if now_word == rand_w: # Объявление о победе
            print(colored('Поздравляю вы победили!!!', 'green'))
            print(colored('Оставшееся здоровье', 'red'), f'{'❤️' * (7 - falls)} ({colored(str(7 - falls), 'red')})')
            break

        if falls == 7:
            print(colored('Вы проиграли!', 'red'))
            print(f'Правильное слово: {rand_w}')
            break


def start():
    print(colored('Добро пожаловать в игру "Виселица"', 'green'))
    question = input(colored('Хотите сыграть ? (Ответы: да/yes/нет)\n', 'yellow'))

    while  question.lower() in 'да, yes':
        print('', '-' * 100, '', sep='\n')
        game()
        question = input(colored('Хотите сыграть ещё раз? (Ответы: да/yes/нет)', 'yellow'))

    print(colored('Досвидания, надеюсь вы еще венетесь', 'light_cyan'))
